...
public abstract class Resistance implements CharakterElems1 {
	...
	try {
	    ...
	    // init armor
	    changeModValue(getArmorResValue(charakterInCombatBase));
	    return this;
    ...
    protected abstract int getArmorResValue(CharakterInCombatBase charakter);
	...
	try {
	    ...
	    if (charakter.getCharakterEquipment().getArmor() != null) {
		changeModValue(getArmorResValue(charakter));
	    }
	    return this;
	} catch (NoSuchFieldException | SecurityException | IllegalArgumentException | IllegalAccessException e) {
	    e.printStackTrace();
